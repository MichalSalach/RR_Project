...
run_scraper <- function(){
  ...
  for (match in matches){
    ...
    # If an error occurred, assign the result of the tryCatch function to the new_row variable
  ...
  # Additionally save a snapshot of data to the data folder with its timestamp
  write_csv(data, paste0('data/data_', scraping_time, '.csv'))
  
  
  
  # The data should be identical as that scraped with Python, perhaps the date-time column will require
  # turning to string, in order to be displayed correctly on page.
...
# Additionally save a snapshot of data to the data folder with its timestamp
write_csv(data, paste0('data/data_', scraping_time, '.csv'))
write_csv(data, "data/data_current.csv")
